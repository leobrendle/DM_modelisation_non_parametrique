---
title: "TP modélisation non-paramétrique par des fonctions B-splines"
...
L'objectif de ce TP est de modéliser le prix des appartements à Varsovie en utilisant des méthodes non paramétriques. Les variables du jeu de données sont les suivantes : **surface** : surface en mètres carrés, **district** : district (variable facteur à 4 modalités), **n.rooms** : nombre de pièces, **floor** : étage, **construction.date** : année de construction et **areaPerMzloty** : prix au mètre carré (variable à prédire).
...
p2 <- ggplot(WarsawApts, aes(x = n.rooms, y = areaPerMzloty)) +
  ...
  labs(title = "Prix en fonction du nb pièces",
...
p4 <- ggplot(WarsawApts, aes(x = construction.date, y = areaPerMzloty)) +
  ...
  labs(title = "Prix en fonction de l'année",
...
```{r choix_var_univ, echo=FALSE, message=FALSE, warning=FALSE}
...
### 3. Modélisation paramétrique du prix en fonction de la date de construction
...
On commence par normaliser cette variable. Ensuite on définit les modèles GAM avec le paramètre de lissage sp = \( \lambda \) que l'on cherche par validation croisée par blocs.
...
data2 <- data.frame(annee, areaPerMzloty)
...
m_lm <- lm(areaPerMzloty ~ surface + n.rooms + floor + construction.date, data = WarsawApts)
```
...
# Mettre avec noms
resultats <- c("R2" = r_squared, "AIC" = model_aic)
...
\[
\text{areaPerMzloty} = \beta_0 + \beta_{11}\,\text{surface} + \beta_{12}\,\text{surface}^2 + \beta_{21}\,\text{n.rooms} + \beta_{22}\,\text{n.rooms}^2 + \beta_{31}\,\text{floor} + \beta_{32}\,\text{floor}^2 + \beta_{41}\,\text{date} + \beta_{42}\,\text{date}^2 + \varepsilon
\]
...
# Extraire R² et AIC
r_squared <- summary(m_poly)$r.squared
model_aic <- AIC(m_poly)
...
```
Dans ce modèle, le $R^2$ ajusté est meilleur (0.27): le modèle capture plus de structure. En contrepartie, l’interprétation devient difficile et le nombre de paramètres augmente rapidement. L'AIC est plus faible (3581) donc la complexité supplémentaire du modèle est justifiée.
...
### 1. Modèle linéaire avec district
...
# summary(m_lin_qual) (voir annexe)
...
Enfin, la part de variance expliquée par le modèle reste faible (R² ajusté $\approx$ 0.12). Ce modèle linéaire capture donc une partie de la structure, mais il laisse une grande variabilité inexpliquée. Cela justifie d’essayer des modèles plus flexibles, en particulier un modèle GAM avec un effet lissé de `construction.date`.
...
```
La comparaison montre d’abord que la spline commune améliore très nettement le modèle par rapport à la relation linéaire (p < 2.2e-16), ce qui confirme le caractère non linéaire de l’effet de l’année de construction. La comparaison suivante indique qu’autoriser une spline différente selon le district améliore également l’ajustement, mais de manière plus limitée (p $\approx$ 0.073). Ainsi, la non-linéarité est clairement établie, et une variation de la forme par district est plausible, même si l’évidence statistique reste marginale.


## Conclusion

Dans ce travail, nous avons mis en évidence l’intérêt des méthodes non paramétriques pour modéliser le prix au m² des appartements de Varsovie. L’analyse univariée a montré que l’année de construction est la variable présentant la relation la plus structurée avec le prix, et que les splines pénalisées capturent cette non-linéarité de manière bien plus flexible que les modèles paramétriques polynomiaux. L’extension multivariée confirme l’apport des GAM, qui offrent un bon compromis entre complexité et capacité d’adaptation. Enfin, l’intégration de la variable qualitative district montre que la relation entre l’année de construction et le prix varie selon les zones, ce qui souligne l’hétérogénéité géographique du marché immobilier. Les modèles GAM apparaissent ainsi comme les mieux adaptés pour décrire et prédire ces données, grâce à leur capacité à combiner effets lissés et effets catégoriels.

# Annexes : summary des différents modèles {-}
...
## Modèle linéaire avec district

```{r}
print(summary(m_lin_qual))
```

## GAM avec spline commune

```{r}
print(summary(m_gam_commune))
```

## GAM par district

```{r}
print(summary(m_gam_par_district))
```
